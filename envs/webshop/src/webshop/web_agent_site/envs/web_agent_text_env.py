...
class WebAgentTextEnv(gym.Env):
    ...
    def reset(self, session=None, instruction_text=None):
        ...
        self.prev_obs = [obs]
        self.prev_actions = []
        self.browser.server.sub_reward = []
        return obs, None
...
class SimServer:
    ...
    def item_page(self, session_id, **kwargs):
        ...
        all_options = list(single_product['options'].values())
        cur_options = list(session['options'].values())
        
        # generate all possuble combinations of options
        all_combinations = list(itertools.product(*all_options))
        maxr = -1 # whether contain goal item in search page
        if len(all_combinations) > 0:
            for c in all_combinations:
                price = single_product['pricing'][0]
